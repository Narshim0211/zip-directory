 ...
 * Clean, futuristic design with complete data separation.
 */
const VisitorInbox = () => {
  const { user } = useAuth();
        ...
        {loading ? (
          ...
          >
            ...
            <p style={{ color: '#888', marginBottom: '24px' }}>
              Start messaging salons, stylists, or other users to begin conversations
            </p>
          </div>
        ) : (
          /* Thread List */
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {threads.map((thread) => {
              ...
              const displayName = thread.threadType === 'business'
                ? thread.business?.businessName || 'Business'
                : thread.owner?.firstName
                  ? `${thread.owner.firstName} ${thread.owner.lastName || ''}`.trim()
                  : 'User';

              const displaySubtext = thread.threadType === 'business'
                ? thread.business?.location || 'Salon'
                : thread.owner?.handle || '@user';
